...
func step(actor: CharacterBody2D, input: Dictionary, delta: float) -> void:
	...
	_apply_gravity(actor, delta, jump_held, jump_released)
	_apply_horizontal(actor, delta, dir)
	_apply_jump_buffer(actor)
...
func _apply_gravity(actor: CharacterBody2D, delta: float, jump_held: bool, jump_released: bool) -> void:
	if actor.is_on_floor():
		return
...
func _apply_horizontal(actor: CharacterBody2D, delta: float, dir: float) -> void:
	if dir != 0.0:
...
func _apply_jump_buffer(actor: CharacterBody2D) -> void:
	if jump_buffer <= 0.0:
		return
